...
public class AnalogArrayList<T> {
    ...
    public boolean add(T elementOfArray) {//добавление элемента в массив
        if (pointer == arrayObjects.length - 1) { //если достигли предела границы массива, то
            ...
            //заполненного массива в новый увеличинный массив
    ...
    public boolean remove(int index) { //удаление элемента массива по индексу
        ...
        if (arrayObjects.length > DEFAULT_CAPACITY && pointer < arrayObjects.length / REDUCTION_SIZE) { //если элементов
            //в масиве меньше, чем его длина в REDUCTION_SIZE раз, то уменьшаем массив в 2 раза (для экономии памяти). Но
            //уменьшее массива до номинального размера.
    ...
    public int size() {//метод возврата размера массива
        return pointer;//размер равен предельному указателю предедльной границы массива
    }
    ...
    public boolean contains(T elementOfArray) {//метод для сравнения обьектов в массиве
        for (Object element : arrayObjects) {//циклом for-each (для каждого єлемента из массива) проходим по массиву
            if (element.equals(elementOfArray)) {//если єлемент массива будет равен сравниваемому єлементу, то
                return true;
            }
        }
        return false;
